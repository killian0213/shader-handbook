// Image (image) — [TWITCH] Pounding Aldebaran by evvvvil
// https://www.shadertoy.com/view/tt3SWB
...
{ 
  ...
  t.x=abs(t.x)-.3; //Onion trick to make the sphere hollow, carving holes basically
  ...
  t=t.x<h.x?t:h; //Merge all geometries from both black and white materials.
...
{ 
  ...
  h=pounder(p+vec3(2,5,2)); //MAKE IT POUND! Yeah could have been boring but then we have major earth pounding action, who doesn't love pumping hey? 
  ...
  t=t.x<h.x?t:h; //Add particles to rest
  return t;
}
