// Image (image) — Precalculated Voronoi Heightmap by Shane
...
/*
	...
	it as a heightmap for raymarching purposes. The example itself is nothing exciting, but 
    it  shows that you can now calculate things like multiple terrain layers - or anything 
    you can dream up - in a single pass. In fact, if you put together a simple resizing 
    system, you could get it done in a single frame.

	You can find the texture creation behind the "Buf A" tab. For the record, I haven't	
    properly investigated correct procedures yet, so there's probably a better way to 
    preload a heightmap. Ideally, it'd be nice to render to a fixed sized buffer, like 512 
    by 512 for instance, which would be especially helpful when moving to fullscreen, but I 
    don't think that's possible at present? Either way, the framerate is much, much better 
    in a sub-1000-px sized canvas than it would be if you had to do the same without 
    multipass.

	By the way, thanks to "poljere," "iq" -- and whoever else was involved -- for providing 
	multiple-pass functionality. Putting that together would have been a backend nightmare, 
...
float heightMap( in vec2 p ) { 
    ...
    return texture(iChannel0, fract(p/2.), -100.).x;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec3 tex = texture(iChannel1, uv).xyz; tex *= tex;
    col = (tex.xyz)*h*2.;
	#endif
