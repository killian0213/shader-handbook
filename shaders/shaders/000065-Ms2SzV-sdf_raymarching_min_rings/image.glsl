// Image (image) — sdf raymarching: min rings by hornet
...
const float STEP_MULT = 32.0 / NUM_ITERATIONS_F;
...
// ============================================================
// most primitive distance-functions honestly stolen from https://iquilezles.org/articles/distfunctions
...
vec4 raymarch( inout vec3 p, inout vec3 dir, out int out_steps, out float dmin )
{
	...
	for ( int i=0; i<NUM_ITERATIONS; i++ )
	{
		...
		{
			float dt = STEP_MIN + STEP_MULT * d.x; //note: constant-multiply to compensate for distorted space, actual dist < dist - could use gradient-approximation instead? (see iq)
			p += dir * dt;
			rdt += dt;
		}
	}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	outcol.rgb = max( vec3(0,0,0), outcol.rgb );
