// Image (image) — [SIG15] The DeLorean by P_Malin
...
{
    pixelRandom = noise(fragCoord + iTime);
	vec2 vUV = (fragCoord.xy) / iResolution.xy;

    //gTime = iTime;
    gTime = max(0.0, iTime - 5.0);
    ...
    float fAngle = radians(-30.0) + sin(iTime * 0.25) * 0.2;
...
void GetSurfaceInfo(out vec3 vOutAlbedo, out vec3 vOutR0, out float fOutSmoothness, out vec3 vOutBumpNormal, const in C_Intersection intersection )
{
	...
	else if(intersection.fObjectId == MAT_CAR_WHEEL)
	{
        ...
        {
            vOutAlbedo = texture( iChannel0, vec2(r, theta / 3.14) ).rgb;
...
{	
    ...
    vec3 vResult = texture( iChannel1, vDir ).rgb;
...
float noise( in vec2 x )
{
	...
	vec2 uv = (p.xy) + f.xy;
	return textureLod( iChannel2, (uv+ 0.5)/256.0, 0.0 ).x;
}
...
{   
	...
	//vec3 vRayOrigin = g_vCameraPos + fragRayOri;
    vec3 vRayOrigin = g_vCarPos + vec3(2.0, 1.0, 3.5) + fragRayOri;
