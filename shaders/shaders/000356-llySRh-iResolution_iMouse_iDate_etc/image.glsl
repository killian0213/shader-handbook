// Image (image) — iResolution, iMouse, iDate, etc by FabriceNeyret2
...
// --- short approx hue/HSV -------------- https://www.shadertoy.com/view/ll2cDc
...
vec4 rgb2hsv(vec4 C) {
    float M =     max(C.r,max(C.g,C.b)),
          d = M - min(C.r,min(C.g,C.b)),
          h = d ==  0. ? 0.
            : M == C.r ? mod((C.g-C.b)/d , 6.)
            : M == C.g ?     (C.b-C.r)/d + 2.
            :                (C.r-C.g)/d + 4.; 
    return vec4( h/6., M>0.? d/M :0., M , 0); // h in [0,1]
              // 60.*h                        // h in [0,360]
}
...
void mainImage( out vec4 O,  vec2 uv ) // === demos it all ==================================
{
  ...
  //O += pInt(U, iResolution.z);  U.y += .8;
    ...
    if (t) O   += S( line( U,vec2(0, .1),vec2(2, .1)), 3., 2./R.y ); // toggle
    if (c) O.r += S( line( U,vec2(0,-.1),vec2(2,-.1)), 3., 5./R.y ); // just clicked
