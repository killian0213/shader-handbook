// Image (image) — post: bokeh blur, hexagon 5pass by hornet
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    {
     	sumcol += srgb2lin( texture( iChannel1, p, -10.0 ).rgb);
        p += stepvec;
    }
    sumcol /= float(NUM_SAMPLES);
    
    vec3 s2 = srgb2lin( texture( iChannel2, uv).rgb );
    vec3 s3 = srgb2lin( texture( iChannel3, uv).rgb );

    //DEBUG
    //fragColor = texture( iChannel1, uv); return; //DBG
    //fragColor = vec4(sumcol,1.0); return; //DBG
    //fragColor = texture( iChannel0, uv); return; //DBG
