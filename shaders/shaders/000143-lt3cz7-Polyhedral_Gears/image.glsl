// Image (image) — Polyhedral Gears by Shane
...
/*
// Tetrahedral normal, to save a couple of "map" calls. Courtesy of IQ. In instances where 
// there's no descernible aesthetic difference between it and the six tap version, it's 
// worth using.
vec3 calcNormal(in vec3 p){

    // Note the slightly increased sampling distance, to alleviate artifacts due to hit 
    // point inaccuracies.
...
vec3 calcNormal(in vec3 p) {
	...
	return normalize(vec3(map(p + e.xyy) - map(p - e.xyy), map(p + e.yxy) - 
                          map(p - e.yxy),	map(p + e.yyx) - map(p - e.yyx)));
}
*/
...
vec3 calcNormal(vec3 p, inout float edge, inout float crv, float t) { 
    ...
    //iResolution.y - Depending how you want different resolutions to look.
    e = vec2(.001, 0); 
	d1 = map(p + e.xyy), d2 = map(p - e.xyy);
	d3 = map(p + e.yxy), d4 = map(p - e.yxy);
	d5 = map(p + e.yyx), d6 = map(p - e.yyx);
...
// Ambient occlusion, for that self shadowed look. Based on the original by XT95. 
// I love this function and have been looking for an excuse to use it. For a better version,
// and usage, refer to XT95's examples below:
//
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(t < FAR){
        ...
        sceneCol = col*(diff*shd + .25); 
        
        // Specular term.
        if(svObjID == 3.) sceneCol += col*vec3(1, .6, .2).zyx*spec*shd*.25; // Less specular on the back plane.
        else sceneCol += col*vec3(.5, .75, 1.)*spec*shd*2.;
        ...
        sceneCol *= atten*ao; // Applying the light falloff, shadows and AO.
