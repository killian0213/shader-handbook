// Buf C (buffer) — Smoke remake by Ultraviolet
...
float rhs(int x,int y){
    ...
    vec2 cc = gl_FragCoord.xy+vec2(x,y);
    //if(any(greaterThan(cc, iResolution.xy)) || any(lessThan(cc, vec2(0.))))
    //    return 0.;
    
	//return texelFetch(iChannel0,clamp(ivec2(gl_FragCoord)+ivec2(x,y), ivec2(0), ivec2(iResolution.xy)-1),0).x;
	return texelFetch(iChannel0,ivec2(cc),0).x * (1.-step(iResolution.x, cc.x))  * (1.-step(iResolution.y, cc.y)) * (step(0., cc.x)) * (step(0., cc.y));
}
...
float p0c(int x,int y){
	return texelFetch(iChannel1,clamp(ivec2(gl_FragCoord)+ivec2(x,y), ivec2(0), ivec2(iResolution.xy)-1),0).x;
    
    //vec2 cc = gl_FragCoord.xy+vec2(x,y);
	//return texelFetch(iChannel1,ivec2(cc),0).x * (1.-step(iResolution.x, cc.x))  * (1.-step(iResolution.y, cc.y)) * (step(0., cc.x)) * (step(0., cc.y));
