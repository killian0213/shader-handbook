// Image (image) — Mobius trans in hyper 3-space by neozhaoliang
...
bool trace(vec3 ro, vec3 rd, out float depth, out vec2 p, out float pint)
{
    depth = MIN_TRACE_DIST;
    ...
    // A missed ray has no surface position, normal, or grid intensity.
    if (!hit)
    {
        p = vec2(0.0);
        pint = 0.0;
    ...
    // Keep the original mathematical mapping and texture coordinates.
    if (b_parabolic)
    {
        if (b_apply)
            pos /= dot(pos, pos);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // This must remain a fixed point light. The original background branch
    // overwrote it after a missed ray, corrupting later AA samples.
    const vec3 lightOffset = vec3(0.2, 0.8, -0.2);
    vec3 lightPos = ro + lightOffset;

    for (int ii = 0; ii < AA; ii++)
    {
        for (int jj = 0; jj < AA; jj++)
        {
            // Symmetric sub-pixel sampling around Shadertoy's pixel center.
            vec2 offset = (vec2(float(ii), float(jj)) + 0.5) / float(AA) - 0.5;
            ...
            // Preserve the intended sky calculation, but use a local direction
            // instead of destroying the point-light position.
            vec3 skyDir = normalize(lightPos - (ro + rd * FAR));
            float skyMix = clamp(0.5 - 0.5 * skyDir.y, 0.0, 1.0);
            vec3 bg = mix(vec3(0.5, 0.7, 1.0),
                          vec3(1.0, 0.5, 0.6),
                          skyMix) * 1.3;
            ...
            float pint;
            float t;
            bool hit = trace(ro, rd, t, p, pint);

            // A missed ray is background only. It must not evaluate normals,
            // AO, shadows, or surface material at a fictitious FAR position.
            vec3 col = bg;
            ...
            if (hit)
            {
                ...
                float at = 2.2 / (1.0 + lightDist * 0.1
                                      + lightDist * lightDist * 0.05);
                float ao = calcAO(pos, nor);
                float sh = softShadow(pos, ld, 0.04, lightDist, 8.0);

                float diff = clamp(dot(nor, ld), 0.0, 1.0);
                float spec = max(0.0, dot(reflect(-ld, nor), -rd));
                spec = pow(spec, 50.0);

                vec3 surfaceCol = diff * 2.5 * ocol
                                + vec3(0.6, 0.8, 0.8) * spec * 2.0;
                surfaceCol *= ao * sh * at;

                // Keep the original distance-fog behavior for actual hits.
                float fog = smoothstep(0.0, FAR - 2.0, t);
                col = mix(clamp(surfaceCol, 0.0, 1.0), bg, fog);
            }
    ...
    tot /= float(AA * AA);
    fragColor = vec4(sqrt(clamp(tot, 0.0, 1.0)), 1.0);
}
