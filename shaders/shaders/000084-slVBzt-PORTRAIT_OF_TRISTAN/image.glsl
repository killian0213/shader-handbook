// Image (image) — PORTRAIT OF TRISTAN by alro
...
// If not animated
const float lightPosition = -2.0;
const float lightHeight = 0.6;
...
vec3 getDetailExtrusion(vec3 p, vec3 normal){
    ...
    // Eyebrows
    if(uvv.x > 0.0 && uvv.x < 1.0 && uvv.y > 0.0 && uvv.y < 1.0){
        detail += w * 0.75*texture(iChannel3, 2.0*uvv-vec2(0.975, 1.155)).b;
        detail += w * 0.75*texture(iChannel3, vec2(-1, 1) * (2.0*uvv-vec2(0.975, 1.155))).b;
    }
...
vec3 shadeEyes(vec3 p, vec3 normal){
    ...
    vec3 col = 0.7 * vec3(1.0, 0.75, 0.7);
    ...
    col = mix(col, vec3(0.25, 0.15, 0.15)*col, smoothstep(0.02, -0.025, eyeDist));
    return col;
}
...
float getShadow(vec2 uv, vec3 p, vec3 cameraPos, vec3 lightDir, vec2 res, mat4 invViewMatrix){
    ...
    for(int i = 0; i < SAMPLES; i++) {
        ...
        vec4 clip = perspectiveMatrix * (invViewMatrix * vec4(pos, 1.0));
        clip.xyz /= clip.w;  
...
vec3 shading(vec3 p, vec3 n, vec3 org, vec3 rayDir, vec3 geoNormal, float shadow, vec3 lightDir){
    ...
    if(uvv.x > 0.0 && uvv.x < 1.0 && uvv.y > 0.0 && uvv.y < 1.0){
        ...
        float eyebrows = max(texture(iChannel3, 2.0*uvv-vec2(0.975, 1.155)).b, 
                             texture(iChannel3, vec2(-1, 1) * (2.0 * uvv - vec2(1.025, 1.155))).b);
    
        albedo = mix(albedo, 0.175*vec3(0.4, 0.2, 0.1), w * smoothstep(0.0, 1.0, 200.0*eyebrows));
    
        albedo = saturate(albedo);
    }
    
    float hairArea = smoothstep(0.25, 1.25, getHairArea(p));
    float hair = hairArea * (0.6 + smoothstep(0.25, 1.95, saturate(1.0-worley(p, 128.0))));
    albedo = mix(albedo, vec3(0.04, 0.02, 0.01), saturate(hair));
    ...
    roughness = 0.25;
    roughness = mix(0.5, roughness, smoothstep(0.0, 0.01, collar));
    ...
    vec3 col = getOcclusion(p) * radiance * (shadow * specular + albedo * (ambient + shadow * diffuse));
    if(eyeballDist < 0.00175){
        col = mix(vec3(0), col, smoothstep(0.0005, 0.0015, getPupilSDF(q)));
    }
    return col;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    #ifdef SHADOWS
        shadow = getShadow(uv, p, cameraPos, lightDir, iResolution.xy, inverse(viewMatrix4));
    #endif
    
        col = shading(p, detailNormal, cameraPos, rayDir, geoNormal, shadow, lightDir);
    ...
    vec3 eyeP = vec3(-1e5);
    vec3 eyeNormal = vec3(1,0,0);
    vec3 e = vec3(-0.541, -0.008, 0.255);
    vec2 eyeIntersections = sphereIntersect(cameraPos-e, rayDir, 0.14);
    bool right = eyeIntersections.x > 0.0 && eyeIntersections.y > eyeIntersections.x && eyeIntersections.x < data.a;
    if(right){
        eyeP = cameraPos + rayDir * eyeIntersections.x;
        eyeNormal = normalize(eyeP - e);
    }
    
    e = vec3(-0.541, -0.008, -0.255);
    eyeIntersections = sphereIntersect(cameraPos-e, rayDir, 0.14);
    bool left = eyeIntersections.x > 0.0 && eyeIntersections.y > eyeIntersections.x && eyeIntersections.x < data.a;
    if(left){
        eyeP = cameraPos + rayDir * eyeIntersections.x;
        eyeNormal = normalize(eyeP - e);
    }
    if(left || right){
        col += shadow * BRDF(eyeP, eyeNormal, -rayDir, lightDir, F0, roughness) * dot_c(eyeNormal, lightDir);
    }
