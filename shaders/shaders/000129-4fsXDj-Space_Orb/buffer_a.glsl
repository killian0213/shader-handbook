// Buffer A (buffer) — Space Orb by cmzw
// https://www.shadertoy.com/view/4fsXDj
...
{    
    ...
    const vec2 off = vec2(.05, .01)*.5; const vec2 dp = vec2(.7, .5);
    ...
    {   
        ...
        float am = ST(1.32, 1.3, dduv); sp *= am;
        ...
        // terrain noise
        float t = floor(-.8+wn(
            fract((spm*.5+off*iTime))
            ,.75, TERRAIN_SCALE, TERRAIN_SEED, TERRAIN_WARP_STRENGTH) * 32.) / 32.;

        // clouds noise
        float c = floor(wn(
            fract((spm*.45+off*iTime))
            ,.65, CLOUDS_SCALE, CLOUDS_SEED, CLOUDS_WARP_STRENGTH) * 24.) / 24.;
            
        // clouds noise shadows
        float cs = floor(wn(
            fract(((spm+vec2(.015))*.45+off*iTime))
            ,.65, CLOUDS_SCALE, CLOUDS_SEED, CLOUDS_WARP_STRENGTH) * 24.) / 24.;
