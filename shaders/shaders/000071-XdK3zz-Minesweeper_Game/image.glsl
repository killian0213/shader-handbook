// Image (image) — Minesweeper Game by demofox
...
vec3 BackgroundPixel (in vec2 uv)
{
    ...
    vec2 offsetG = vec2(sin(iTime*1.534), cos(iTime*1.453)) * 10.0 / iChannelResolution[1].xy;
    vec2 offsetB = vec2(sin(iTime*1.756), cos(iTime*1.381)) * 10.0 / iChannelResolution[1].xy;
        
   	vec3 ret;    
    ret.r = texture( iChannel1, uv + vec2(distortX, distortY) ).r;
    ret.g = texture( iChannel1, uv + vec2(distortX, distortY) + offsetG ).r;
    ret.b = texture( iChannel1, uv + vec2(distortX, distortY) + offsetB ).b;        
    return ret;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 cellData = texture( iChannel0, (cell+0.5) / iChannelResolution[0].xy, -100.0 );
    ...
    //pixelColor = texture(iChannel0, uv).rbg;
