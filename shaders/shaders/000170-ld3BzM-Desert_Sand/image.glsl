// Image (image) — Desert Sand by Shane
...
float sandL(vec2 p){
    ...
    //float c = mix(grad1, grad2, dot(sin(q*12. - cos(q.yx*12.)), vec2(.25)) + .5);
...
float trace(in vec3 ro, in vec3 rd){
    ...
    for(int i = 0; i<96; i++){
        ...
        if(abs(h)<.001*(t*.125 + 1.) || t>FAR) break; // Alternative: 0.001*max(t*.25, 1.), etc.
...
// Cheap shadows are the bain of my raymarching existence, since trying to alleviate artifacts is 
// an excercise in futility. In fact, I'd almost say, shadowing - in a setting like this - with limited  
