// Buffer A (buffer) — single pass CFD by flockaroo
...
///// strange behaviour here: iChannelResolution[0] gives different results than textureSize(iChannel0,0) ?!?!?
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    {
        //if ( sc > 0.7 ) break;  /// hmmm moved this to bottom - otherwise was optimized out by AMD compiler?!?! (but only when using iChannelResolution[0] - whuuut?!?!)
        ...
        for(int i=0;i<RotNum;i++)
        {
      	...
      	sc*=2.0;
        if ( sc > 0.7 ) break;   /// hmmm moved this here from top due to AMD-compatibility?!
    }
