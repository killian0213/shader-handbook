// Image (image) — Abstract Corridor by Shane
...
/*
    ...
    Using Shadertoy user Nimitz's triangle noise idea and his curvature function to fake 
    an abstract, flat-shaded, point-lit, mesh look.

	It's a slightly trimmed back, and hopefully, much quicker version my previous tunnel
    example... which is not interesting enough to link to. :)
...
// The triangle function that Shadertoy user Nimitz has used in various triangle noise 
// demonstrations. See Xyptonjtroz - Very cool. Anyway, it's not really being used to its 
// full potential here.
vec3 tri(in vec3 x){return abs(x-floor(x)-.5);} // Triangle function.

// The function used to perturb the walls of the cavern: There are infinite possibities, 
// but this one is just a cheap...ish routine - based on the triangle function - to give a 
// subtle jaggedness. Not very fancy, but it does a surprizingly good job at laying the 
// foundations for a sharpish rock face. Obviously, more layers would be more convincing. 
// However, this is a GPU-draining distance function, so the finer details 
// are bump mapped.
...
// Standard tunnel distance function with some perturbation thrown into the mix. A floor 
// has been worked in also. A tunnel is just a tube with a smoothly shifting center as you 
// traverse lengthwise.  The walls of the tube are perturbed by a pretty cheap 3D surface 
// function.
float map(vec3 p){
...
/*
    // Rounded square tunnel using Minkowski distance: 
    // pow(pow(abs(tun.x), n), pow(abs(tun.y), n), 1/n)
    vec2 tun = abs(p.xy - path(p.z))*vec2(0.5, 0.7071);
    tun = pow(tun, vec2(4.));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    // Light positioning. One is a little behind the camera, and the other is further down 
    // the tunnel.
 	vec3 light_pos = camPos + vec3(0.0, 0.125, -0.125);// Put it a in front of the camera.
	...
	if(dt<0.005){
        ...
        //
        // Shiny.
        //sceneCol = texCol*((diff + diff2)*vec3(1.0, 0.95, 0.9) + ambience + 
        //                    fre*fre*texCol) + (spec + spec2);
        // Abstract pen and ink?
        //float c = getGrey(texCol)*((diff + diff2)*1.75 + ambience + fre*fre) + 
        //                           (spec + spec2)*0.75;
    ...
    // Edit: No gamma correction -- I can't remember whether it was a style choice, or 
    // whether I forgot at the time, but you should always gamma correct. In this case, 
    // just think of it as rough gamma correction on a postprocessed color: 
    // sceneCol = sqrt(sceneCol*sceneCol); :D
