// Image (image) — Nebulous Tunnel by Shane
// https://www.shadertoy.com/view/ltfBzM
...
float map(vec3 p) {
    ...
    return smax(tn - .075, -length(p.xy) + 1., 2.);
    #else
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    for (int i=0; i<64; i++) {
        ...
        if(d<.001*(1. + t*.05) || td>1. || t>FAR) break;
        ...
        col += w*d*(diff*vec3(1, .97, .92)*3. + 2.5);
    ...
    col = mix(col, col + sky*(1. - col), smoothstep(0., .55, t/FAR));
