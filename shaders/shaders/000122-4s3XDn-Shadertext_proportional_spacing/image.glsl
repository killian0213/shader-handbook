// Image (image) — Shadertext proportional spacing by Andre
...
#define g_ ch(gg,0.7);
#define h_ ch(hh,0.7);
#define i_ ch(ii,0.33);
#define j_ ch(jj,0.33);
#define k_ ch(kk,0.7);
#define l_ ch(ll,0.33);
#define m_ ch(mm,0.9);
#define n_ ch(nn,0.7);
#define o_ ch(oo,0.7);
#define p_ ch(pp,0.7);
...
#define _comma ch(comma,0.33);
#define _exclam ch(exclam,0.33);
#define _question ch(question,0.8);
#define _open1 ch(open1,0.7);
#define _close1 ch(close1,0.7);
#define _dotdot ch(dotdot,0.33);
#define _dotcomma ch(dotcomma,0.33);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec2 uv = (fragCoord+mouseOffs-.5*iResolution.xy) / iResolution.x * 22.0;
#if (MOVE)
    scale += 1.0-1.4*cos(iTime*.17);
    uv.x += sin(iTime);
    uv.y += cos(iTime*2.13);
    float angle = sin(iTime*.073);
    mat2 rotate = mat2(
        cos(angle),-sin(angle),
        sin(angle), cos(angle));
    uv *= rotate;
    ...
    float px = 42.0/iResolution.x*pow(scale,1.3);
    float x = 100.;
    float cp = 0.;
    ...
    // Wobbly font
    // float weight = 0.05+sin(length(-10.0+uv*3.9-10.0*sin(.09*iTime)))/50.;
    
    float weight = 0.01+us.w*min(iTime*.02-.05,0.05);//+.03*length(sin(uv*6.+.3*iTime));//+0.02-0.06*cos(iTime*.4+1.);
    if (ms>0.) {
