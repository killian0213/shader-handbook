// Image (image) — Refraction blobs by LeWiZ
...
void mainVR( out vec4 fragColor, in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir )
{
    ...
    float t = iTime;
    vec3 campos = fragRayOri + vec3( 0.0, 0.0, 8.0 );
    vec3 camtar = campos + 5.0 * fragRayDir;
    
    mat3 camMat = calcLookAtMatrix( campos, camtar, 0.0 );  // 0.0 is the camera roll
	vec3 camdir = normalize( camMat * vec3(xy,2.0) ); // 2.0 is the lens length
