// Buf A (buffer) — Satin Flow - Dispersion by cornusammonis
...
vec3 advect(vec2 ab, vec2 vUv, vec2 step) {
    ...
    vec3 u =    texture(iChannel0, (aUv)).xyz;
    vec3 u_n =  texture(iChannel0, (aUv+n)).xyz;
    vec3 u_e =  texture(iChannel0, (aUv+e)).xyz;
    vec3 u_s =  texture(iChannel0, (aUv+s)).xyz;
    vec3 u_w =  texture(iChannel0, (aUv+w)).xyz;
    vec3 u_nw = texture(iChannel0, (aUv+nw)).xyz;
    vec3 u_sw = texture(iChannel0, (aUv+sw)).xyz;
    vec3 u_ne = texture(iChannel0, (aUv+ne)).xyz;
    vec3 u_se = texture(iChannel0, (aUv+se)).xyz;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 u =    texture(iChannel2, (vUv)).xyz;
    vec3 u_n =  texture(iChannel2, (vUv+texel*n.xy)).xyz;
    vec3 u_e =  texture(iChannel2, (vUv+texel*e.xy)).xyz;
    vec3 u_s =  texture(iChannel2, (vUv+texel*s.xy)).xyz;
    vec3 u_w =  texture(iChannel2, (vUv+texel*w.xy)).xyz;
    vec3 u_nw = texture(iChannel2, (vUv+texel*nw.xy)).xyz;
    vec3 u_sw = texture(iChannel2, (vUv+texel*sw.xy)).xyz;
    vec3 u_ne = texture(iChannel2, (vUv+texel*ne.xy)).xyz;
    vec3 u_se = texture(iChannel2, (vUv+texel*se.xy)).xyz;
    
    vec3 v =    texture(iChannel0, (vUv)).xyz;
    vec3 v_n =  texture(iChannel0, (vUv+texel*n.xy)).xyz;
    vec3 v_e =  texture(iChannel0, (vUv+texel*e.xy)).xyz;
    vec3 v_s =  texture(iChannel0, (vUv+texel*s.xy)).xyz;
    vec3 v_w =  texture(iChannel0, (vUv+texel*w.xy)).xyz;
    vec3 v_nw = texture(iChannel0, (vUv+texel*nw.xy)).xyz;
    vec3 v_sw = texture(iChannel0, (vUv+texel*sw.xy)).xyz;
    vec3 v_ne = texture(iChannel0, (vUv+texel*ne.xy)).xyz;
    vec3 v_se = texture(iChannel0, (vUv+texel*se.xy)).xyz;
    ...
    if(iFrame < 10 || reset()) {
        ...
        fragColor = vec4(mix(v,dv,0.5), gcurve);
    }
