// Image (image) — Brigthton Beach by Dave_Hoskins
// https://www.shadertoy.com/view/lssGW7
...
{
    vec3 ext = exp2(-t*0.0005*vec3(1,1.3,2.)); 
    return col*ext + (1.0-ext)*vec3(1.); // 0.55
}
...
vec3 GetSky(in vec3 rd)
{
	...
	vec3  sky = mix(vec3(.1, .15, .2), vec3(.2, .2, .25), v);
...
bool Scene(in vec3 rO, in vec3 rD, out float resT, out float type )
{
	...
	for( int j=0; j<160; j++ )
	{
		if (t > 500.0) break; // ...Too far awway
	    vec3 p = rO + t*rD;
