// Buffer A (buffer) — Warped Offset Block Corridor by Shane
// https://www.shadertoy.com/view/tXccDj
...
/*
    ...
    liked the way it turned out. It's stitched together from other examples 
    of mine. The object of the exercise was to create a simple scene that 
    gives the illusion of reflective and refractive materials without the
    added difficulty and exorbitant cost.
    
    For anyone curious, the geometry itself was produced via a traversal of
...
float map(vec3 p){
    ...
    // This is a bit of a hack, but it helps avoid unnecessary calculations inside 
    // the open center of the tunnel, which should lift performance... In theory. :)
    if(abs(p.x)<3.5 && abs(p.y)<2.){ gCD = 1e5; return 1.; }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    // Blueish peripheral coloring -- Not absolutely necessary, but the 
    // amateur designer in me thinks it counters the excess warm tones. :)
