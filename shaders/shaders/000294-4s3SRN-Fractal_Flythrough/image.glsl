// Image (image) — Fractal Flythrough by Shane
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    if(t<FAR){
        ...
        nor = texBump(iChannel0, pos*ts, nor, 0.0015); // + step(saveObjID, 1.5)*0.002
        ...
        if(rSaveObjID>1.5 && rSaveObjID<2.5){
            rCol = dot(rCol, vec3(.299, .587, .114))*.7 + rCol*.15;//*.7+.2
        ...
        rCol *= (rDiff + .15)*rAtten; // Reflected color. Not accurate, but close enough.         
    ...
    col = mix(max(col, 0.), vec3(0), 1.-exp(-t*t/FAR/FAR*20.));//smoothstep(0., FAR-20., t)
