// Image (image) — Quasi Infinite Zoom Voronoi  by Shane
...
/*
	...
	This one is based on something I wrote some time ago, but was inspired by 
    Fabrice Neyret's "Infinite Fall" shader. I've aired on the side of caution and 
    called it "quasi infinite", just in case it doesn't adhere to his strict infinite 
    zoom standards. :)

	Seriously though, I put together a couple of overly optimized versions a couple of 
    days ago, just for fun, and Fabrice's comments were pretty helpful. I also liked the 
    way he did the layer rotation in his "Infinite Fall" version, so I'm using that. The 
    rest is stock standard infinite zoom stuff that has been around for years.
	...
	tempting to add a heap of other things, but I wanted to keep the example relatively 
    simple.

	By the way, most of the code is basic bump mapping and lighting. The infinite zoom 
    code takes up just a small portion.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    // Setup: I find 2D bump mapping more intuitive to pretend I'm raytracing, then 
    // lighting a bump mapped plane situated at the origin. Others may disagree. :)  
    vec3 sp = vec3(uv, 0); // Surface posion. Essentially, a screen at the origin.
    vec3 ro = vec3(0, 0, -1); // Camera position, ray origin, etc.
    vec3 rd = normalize(sp - ro); // Unit direction vector.
    vec3 lp = vec3(cos(iTime)*.375, sin(iTime)*0.1, -1); // Light position.
    ...
    // Infinite Zoom.
    //
    // The first three lines are a little difficult to explain without describing what 
    // infinite zooming is in the first place. A lot of it is analogous to fBm; Sum a bunch 
    // of increasing frequencies with decreasing amplitudes.
    //
    // Anyway, the effect is nothing more than a series of layers being expanded from an 
    // initial size to a final size, then being snapped back to its original size to repeat
    // the process again. However, each layer is doing it at diffent incremental stages in 
    // time, which tricks the brain into believing the process is continuous. If you wish to
    // spoil the illusion, simply reduce the layer count. If you wish to completely ruin 
    // the effect, set it to one.
	...
	for (float i = 0.; i<L; i++){
        ...
        // Using the fractional time component to determine the layer frequency. It 
        // increases with time, then snaps back to one in a cyclic fashion.
        ...
        // Layer ampltude component -- Inversely propotional to the frequency, which makes 
        // sense. Because the layers are finite, you need to smoothly interpolate between 
        // them, and the "cos" setup below is just one of many ways to do it.
        ...
        // I had to have a bit of a think as to how to bump map this. Normally, you'd write 
        // a function then call it three times, but that'd be too expensive, so it's all 
        // being done simultaneously.
        //
        // Either way, it's still pretty simple. In addition to accumulating the pixel 
        // value, accumulate sample values just to the left of it and above. The X-gradient
        // and Y-gradient can then be determined outside the loop.
        //
        f += Voronoi(M*sp.xy*e + offs)*a; // Sample value multiplied by the amplitude.
        fx += Voronoi(M*(sp.xy - eps.xy)*e + offs)*a; // Nearby sample in the X-direction.
        fy += Voronoi(M*(sp.xy - eps.yx)*e + offs)*a; // Nearby sample in the Y-direction.
        
        // Sum each amplitude. Used to normalize the results once the loop is complete.
        sum += a;
        
        // Rotating each successive layer is pretty standard, but this is the way Fabrice
        // does it.
        M *= M;
