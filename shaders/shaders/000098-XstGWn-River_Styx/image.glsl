// Image (image) — River Styx by Dave_Hoskins
...
//----------------------------------------------------------------------------------------
// Neat huh? Not mine though, might have been fabrice's
...
float fbm(in vec3 p)
{
    ...
    {
		f += a*noise( p);
        p = p*m*(textureLod(iChannel1, p, 0.0).xzy+1.3);
        a = a*.5;
    }
    return f;
}
...
//----------------------------------------------------------------------------------------
float rayMarch(in vec3 pos, in vec3 dir, vec2 uv)
{
...
{
	vec3 x = texture( sam, p.yz ).xyz*vec3(.3, .5, .3);
	vec3 y = texture( sam, p.zx ).xyz*vec3(.0, .3 , .8);
...
void mainImage( out vec4 outColour, in vec2 coords )
{
    ...
    gTime = iTime*40.0+iMouse.x*3.0+23600.;
    ...
    if (d < 1700.0)
    {
        ...
        {
            dif = vec3(1.,0.0,0.0);
            nor.xz += texture(iChannel0, loc.xz*.0005+gTime*.0002).xy*.2-.1;
           	nor.xz += texture(iChannel3, -loc.xz*.004+off).xy*.8-.4 ;
          	nor = normalize(nor);
            specAmount=3.;
            amb = vec3(.08, .0, .0);
        }
        ...
        float bri = texture(iChannel3, -loc.xz*.005+off).x;
