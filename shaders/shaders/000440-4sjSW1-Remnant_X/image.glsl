// Image (image) — Remnant X by Dave_Hoskins
...
// NEW anti-aliasing without tracing multiple rays
//#define STEREO
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    spotLight = CameraPath(gTime + .03) + vec3(sin(gTime*18.4), cos(gTime*17.98), sin(gTime * 22.53))*.2;
	vec3 col = vec3(0.0);
    ...
    {		
        ...
        // This is an enormous saving as I don't have to trace more rays to get antialiased points
        // UJse the axis of the camera directions to get the for points surrounding positino found....
 #ifdef ANTI_ALIAS
        vec2 a = ret.x*(1.0/iResolution.xy)*2.;
    ...
    col = mix(sky, col, min(exp(-ret.x*.3), 1.0));
    col += vec3(pow(abs(ret.y), 2.5)) * vec3(.01, .0, .03);
