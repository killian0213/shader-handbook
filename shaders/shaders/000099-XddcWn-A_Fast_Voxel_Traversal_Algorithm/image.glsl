// Image (image) — A Fast Voxel Traversal Algorithm by paniq
...
void paint() {
    ...
    vec2 pos = get_origin();
    vec2 q = floor(pos);
    rectangle(q, vec2(1.0));
    set_source_rgba(0.0,0.0,0.0,0.4);
    stroke();

    vec2 p0 = vec2(6.25,3.75);
#if 1
    float a = iTime;
    vec2 p1 = vec2(cos(a),sin(a)) * 8.0;
#endif

    if (iMouse.z > 0.0) {
        p1 = pixel_coord(iMouse.xy) * N;
        p0 = pixel_coord(iMouse.zw) * N;
    }
    ...
    move_to(p0);
    line_to(p1);
    set_source_rgb(vec3(0.0,0.0,0.0));
    stroke();
