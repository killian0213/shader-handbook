// Image (image) — Rounded Voxels by iq
...
vec2  hash22( vec2  n ) { return fract(sin(n)*vec2(3758.5453123)); }
...
vec3 render( in vec3 ro, in vec3 rd, in float ra )
{
    ...
    if( t>0.0 )
    {
        ...
        {
            vec2  aa = hash22( ra + float(j)*vec2(201.19,13.75) );
            float ra = sqrt(aa.y);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	{
		vec4 rr = texelFetch( iChannel1, ivec2(iTime*60.0*vec2(7.0,13.0)+fragCoord+103.3137*float(a)) & 1023, 0  );
        vec2 p = (2.0*(fragCoord+rr.xz)-iResolution.xy)/iResolution.y;

        float time = iTime + 1.0*(0.5/24.0)*rr.y;
