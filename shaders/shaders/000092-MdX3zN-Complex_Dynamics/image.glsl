// Image (image) — Complex Dynamics by iq
...
// * Black: the Julia set Jc (border of Kc)
// * White: c
// * Blue: the two fixed points: z = fc(z)
// * Green, the period 2 fixed points: z = fc(fc(z))
// * Yellow: the Koening coordinates
// * Checkerboard distortion: the Boettcher map phi(z). 
// * Lines: external rays (arg of phi(z))
// * Shadowing: the gradient of the Green's function, log|phi(z)|
//
// Some theory:
//
...
//   shape.
//
// * The period 2 fixed points (green) are always repelling (belong to Jc, not to Kc) except for the sets 
...
float checkersFiltered( in vec2 p )
{
  ...
  vec2 i = (integral(p+0.5*w) -   // definite integral
            integral(p-0.5*w));
  i /= w;                         // average
  float r = i.x + i.y - 2.0*i.x*i.y; // xor pattern
  return 0.5+0.5*r;
}

// https://iquilezles.org/articles/filterableprocedurals/
float linesGradBox( in float p, float N )
{
    float w = max(abs(dFdx(p)), abs(dFdy(p))) + 0.01;
    float a = p + 0.5*w;                        
    float b = p - 0.5*w;           
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for( int i=0; i<512; i++ )
	{
		if( dot(z,z)>10000.0 ) { inside=false; break; }
	...
	if( !inside )
	{
        ...
        vec2 uv = vec2(length(ph),3.0*atan(ph.y,p.x)/3.14159);
        col *= 0.7+0.3*checkersFiltered( uv*8.0 );
        col *= 0.3+0.7*linesGradBox( 16.0*uv.y, 20.0 );
	}
    // interior
	else
	{
