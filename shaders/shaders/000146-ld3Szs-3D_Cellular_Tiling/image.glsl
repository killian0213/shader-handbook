// Image (image) — 3D Cellular Tiling by Shane
...
/*
    ...
    Creating a Voronoi feel with minimal instructions by way of a 3D tile constructed 
    via a simplistic cellular pattern algorithm. It works surprisingly well under various 
    situations, but isn't a replacement for the standard 3D Voronoi algorithm.

	This is a 3D counterpart to my 2D cellular tiling example. The link is below, where I 
    explain the process more thoroughly, for anyone interested. I came up with the idea 
    for a repeatable cellular tile when experimenting with 3D Truchet tiles.

	Naturally, there are a few restrictions. The obvious one is that repeatable tiles with 
    low object density look very repetitive when you zoom out, so that has to be 
    considered. The upside is quasi 3D celluar surfaces that are fast enough to include in 
    a distance function.

	Anyway, the 3D tiling function is explained below. For anyone interested, the scene 
    itself utilizes an oldschool, warping planes trick. Shadertoy user Branch uses it to 
    great effect in the example "18756.2048d," which is well worth the look.
...
// 2x2 matrix rotation. Note the absence of "cos." It's there, but in disguise, and 
// comes courtesy of Fabrice Neyret's "ouside the box" thinking. :)
...
// There's a few simple, warping tricks being employed here. One is the oldscool, 
// "top and bottom planes" via "abs(p.y)." The planes are then twisted about the XY plane 
// with respect to distance using the 2D rotation function, "rot2(p.z/12.)," etc, then 
// wrapped aound a curvy path, "path(p.z)."
//
// Finally, some surface detailing is added with a sinusoidal bottom layer, and the 
// cellular layer over the top of it. Normally, adding cellular layering utterly fries the 
// GPU, but the "cellTile" function used here merely makes it slightly uncomfortable. :)
//
float map(vec3 p){
     ...
     return 2. - abs(p.y) + n + sf; // Double planes, "abs(p.y)," plus surface layers.
...
/*
// Texture bump mapping. Four tri-planar lookups, or 12 texture lookups in total. I tried 
// to make it as concise as possible. Whether that translates to speed, or not, I couldn't 
// say.
...
float trace(in vec3 ro, in vec3 rd){
    ...
    for(int i = 0; i < 80; i++){
        ...
        // as "t" increases. It's a cheap trick that works in most situations... Not all, 
        // though.
...
// symmetrical. Due to  the intricacies of this particular scene, it's kind of needed to 
// reduce jagged effects.
...
// Using a variation of IQ's AO function to calculate thickness, but with the normal 
// flipped. IQ uses a similar setup to calculate SSS... Subsurface AO, I guess you'd 
// call it. :)
...
float softShadow(vec3 ro, vec3 rd, float start, float end, float k){
    ...
    for (int i=0; i<maxIterationsShad; i++){
        ...
        // +=h, +=clamp( h, 0.01, 0.25 ), +=min( h, 0.1 ), +=min(h, stepDist*2.), etc.
        dist += min(h, stepDist);
...
// Ambient occlusion, for that self shadowed look. Based on the original by XT95. I love 
// this function, and in many cases, it gives really, really nice results. For a better 
// version, and usage, refer to XT95's examples below:
//
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	//vec3 camPos = lookAt + vec3(2., 1.5, -1.5); // Camera position.
	
    vec3 camPos = vec3(0, 0, iTime*6.); // Camera position, doubling as the ray origin.
	vec3 lookAt = camPos + vec3(0, 0, .1);  // "Look At" position.

 
    // Light positioning. One is a little behind the camera, and the other is further 
    // down the tunnel.
    ...
    // function is synchronized with the distance function. Change to "path2" to traverse 
    // the other tunnel.
    ...
    // Standard ray marching routine. I find that some system setups don't like anything 
    // other than a "break" statement (by itself) to exit. 
	...
	if(t<FAR){
    	...
    	/////////   
        // Translucency, courtesy of a mixture XT95 and IQ;s procedures - 
        // See the "thickness" function.
    	...
    	// Darkening the crevices. Otherwise known as cheap, scientifically-incorrect 
        // shadowing.	
	    float shading = 1.;// crv*0.5+0.5; 
    	
        // Shadows - They didn't add enough aesthetic value to justify the GPU drain, 
        // so they didn't make the cut.
        //shading *= softShadow(sp, ld, 0.05, distlpsp, 8.);
    	
    	// Combining the above terms to produce the final color. It was based more on 
        // acheiving a certain aesthetic than science.
        ...
        /*
        // Cool blue hilights. Adapted from numerous examples on here. Kali uses it to 
        // great effect.
        float per = 10.;
        ...
        */
        ...
        //vec3 refCol = vec3(.5, .7, 1)*smoothstep(.2, 1., noise3D((sp + ref*2.)*2.)*.66 + 
        //                   noise3D((sp + ref*2.)*4.)*.34 );
        //sceneCol += refCol*.5;
