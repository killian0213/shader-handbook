// Image (image) — pencil_drawn_crystal by skaplun
// https://www.shadertoy.com/view/styGDw

#define NoiseTex iChannel1
...
void mainImage( out vec4 fragColor, in vec2 fragCoord){
    ...
    vec4 r = distortion(fragCoord * 1.2/sqrt(STROKE_THICKNESS))
           - distortion(fragCoord * 1.2/sqrt(STROKE_THICKNESS) + vec2(1, -1) * 1.5);
...
#define CS(ang) cos(ang - vec2(0, 1.6))
    ...
    fragColor.xyz *= 1. - saturate(mix(hatchesSum/actualHatchesCount, strongestHatch, .5));
