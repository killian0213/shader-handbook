// Cube A (cubemap) — Alpine Flyby 2.0 by loicvdb
// https://www.shadertoy.com/view/4c3GD4

// radiance cache + heightmap

// cubemap is fp16 1024x1024 so it's faster than buffers, although a bit hacky
...
void mainCubemap(out vec4 o, vec2 u, vec3 ro, vec3 rd)
{
    o = texture(iChannel0, rd, 0.0);
    
    vec2 uv = 2.0 * u / iResolution.xy - 1.0;
    
    // update heightmap on first frame
    if (rd.z > rd.x && rd.z > rd.y && iFrame < 10)
    {
        float height = siren(uv);
        
        // nasty screenspace derivative hack
        float dx = dFdx(height) / dFdx(uv.x);
        float dy = dFdx(height) / dFdy(uv.y);
    ...
    {
        // TODO
        // Edit : I got lazy, looks good enough
    }
}