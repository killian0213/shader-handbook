// Buf A (buffer) — Gem Bloom FX by TimoKinnunen
...
vec3 lighting(in Hit h) {
  if (h.ray.len > MAX_TRACE_DIST) return pow(textureLod(iChannel0, -h.ray.dir, 0.).rgb,vec3(0.4))*.25;
