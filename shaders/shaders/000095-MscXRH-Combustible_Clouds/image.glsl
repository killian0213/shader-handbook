// Image (image) — Combustible Clouds by Shane
...
{  
    ...
    for (int i=0; i<64; i++) {
        ...
        // Layer color: Note that I'm only adding a scalar value, in this case,
        // but you can add color combinations.
        vec3 lCol = vec3(1, .7, .4)*w*(1.+ diff*.5 + spec*.5)*atten;
        // Optional extra: Color-based jittering. Roughens up the grey clouds that hit the camera lens.
        lCol += (fract(rnd*289. + t*41.) - .5)*.02;;

        // Try this instead, to see what it looks like without the fake contrasting. Obviously,
        // much faster.
        //lCol += w*atten*1.25;
        
        // Beer's law.. Not really applied correctly, but the example is fake anyway.
        // Either way, this is the trick to more cloud color variation. :)
        float amount = d*8.;   
        lCol = exp(-lCol*amount)*(1. - exp(-lCol*amount))*.4;
