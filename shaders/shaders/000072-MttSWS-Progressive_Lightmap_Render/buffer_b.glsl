// Buffer B (buffer) — Progressive Lightmap Render by huwb
// https://www.shadertoy.com/view/MttSWS
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Cast a ray in random directions to collect radiance
    for( int i = 0; i < INDIRECT_RAYS; i++ )
    {
        ...
        {
            vec3 throughput = col * 2.0 * dot( n, -dir );
		    vec2 LM_UV_SCALE = LM_RES/iResolution.xy;
            addLight += throughput * textureLod( iChannel0, hituv*LM_UV_SCALE, 0. ).xyz;
        }
    }
