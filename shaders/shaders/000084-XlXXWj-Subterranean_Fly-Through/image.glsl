// Image (image) — Subterranean Fly-Through by Shane
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	if(dt<.0025){
    	...
    	// Combining the above terms to produce the final color. It was based more on achieving a
        ...
        //
        // Cold.
        //sceneCol += (getGrey(texCol)*(diff*0.75 + ambience*0.25)+ spec*texCol*2. + fre*crv*texCol.zyx/.5)*atten;
        //sceneCol += (getGrey(texCol)*(diff2*0.75 + ambience*0.25) + spec2*texCol*2. + fre*crv*texCol.zyx/.5)*atten2;
