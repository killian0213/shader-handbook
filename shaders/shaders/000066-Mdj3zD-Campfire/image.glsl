// Image (image) — Campfire by Dave_Hoskins
...
vec4 Raymarch( in vec3 ro, in vec3 rd, inout int hit, in vec2 fragCoord)
{
	...
	float t = 4.2 + .1 * texture(iChannel0, fragCoord.xy / iChannelResolution[0].xy).y;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 cv = ( cross(cu,cw) );
	vec3 ray = normalize(p.x*cu + p.y*cv + (1.5+(Noise(iTime*.5))*.1) * cw );
	...
	if (hit == 1)
	{
		...
		bri = bri*bri * 7.0;
		float f = Bump(ret.xyz*4.53 - vec3 (0.0, iTime*.37, 0.0))*.8 + Bump(ret.xyz*17.3)*.4;
		f += Bump(ret.xyz * 1.0);
...
//		// Stones...
		...
		vec3 mat = vec3(Bump(ret.xyz * 12.3)*.5+.5) * vec3(1.0, .2+texture(iChannel0, ret.xz*.002).x*.5, .12);
		col = mat * bri + vec3(.9, .6, .3) * spe;
	}else
	{
		// Grab the forest texture...
		vec3 frst = texture(iChannel1, ray-vec3(0.0, .08, 0.0), -100.0).xyz;
		frst = frst*frst;
...
//			// Dodgy re-projection onto floor...
			...
			vec3 mat = texture(iChannel1, pos, -100.0).xyz;
			// Rudimentary bump map...
			vec2 bmp = texture(iChannel0, pos.xz*.02, -100.0).xy;
			...
			float f = Bump(vec3(pos.x, iTime*.024, pos.z) * 21.0);
			f += Bump(vec3(pos.x, iTime*.001, pos.z) * 5.3);
