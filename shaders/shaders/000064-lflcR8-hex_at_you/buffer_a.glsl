// Buffer A (buffer) — hex at you by pb
...
void mainImage( out vec4 O, vec2 U) {
    ...
    for (float i=0.; i<60.; i++) {
        ...
        for (float j=0.; j<7.+sn2; j++) {
            ...
            dotp = min(max(1./dot(w,w),.1-.05*sn2),1e4*(1.-sn2)+7.*sn2);
        ...
        c +=
             .6e-1* 
             mix( vec3(1), H(M(sc),sn2),.9)  * exp(-i*i*stepsize*max(1e1,sn2*2e1));
    }
