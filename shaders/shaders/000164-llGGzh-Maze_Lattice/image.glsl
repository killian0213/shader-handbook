// Image (image) — Maze Lattice by Shane
...
float map(in vec3 p) {
   ...
   q.x = fract(sin(mod(dot(ip + q*.5, vec3(111.67, 147.31, 27.53)), 6.2831))*43758.5453);
...
float trace(vec3 ro, vec3 rd){
    ...
    for(int i=0; i<72; i++){
        float d = map(ro + rd*t);
        if (abs(d)<.002*(t*.05 + 1.) || t>FAR) break;
        t += d;
    } 
    return min(t, FAR);
}
...
float hexTruchet(in vec2 p) { 
    ...
    c = fract(cos(mod(dot(h, vec2(41.13, 289.57)), 6.2831))*43758.5453); // Reusing "c."
...
float bumpFunc(vec3 p, vec3 n){
    ...
    vec2 sc = cos(mod(hexTruchet(p.xy)*6.2831*vec2(2, 4), 6.2831));
    return clamp(dot(sc, vec2(.6)) + .5, 0., 1.);

}

// Standard function-based bump mapping function. This is the cheaper four tap 
// version. There's a six tap version (samples taken from either side of each axis), 
// but this works well enough.
vec3 bumpMap(in vec3 p, in vec3 n, float bumpfactor){
    
    // Larger sample distances give a less defined bump, but can sometimes 
    // lessen the aliasing.   
    ...
    // Adjusting the tangent vector so that it's perpendicular to the normal. 
    // It's some kind of orthogonal space fix using the Gram-Schmidt process, 
    // or something to that effect.
    grad -= n*dot(n, grad);          
    
    // Applying the gradient vector to the normal. Larger bump factors make 
    // things more bumpy.   
...
float n2D(vec2 p) {
	...
	return dot(mat2(fract(sin(mod(vec4(0, 41, 289, 330) + dot(i, vec2(41, 289)), 6.2831))*
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    //if(t<FAR){
        ...
        //
        float rt = refTrace(sp + sn*.005, ref); // Raymarch from "sp" in the reflected direction.
        float rEdge;
