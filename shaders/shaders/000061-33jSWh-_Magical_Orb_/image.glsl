// Image (image) — 🪄🔮✨ Magical Orb 🪄🔮✨ by chronos
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    time = iTime;
    {
        ...
        if(y2 <= 4.)
        {
            ...
            t = a 
                + texelFetch(iChannel0, (ivec2(fragCoord)+iFrame*335)%1024, 0).a*.01
            ;
    ...
    color *= 255.;
    color += 5.*(texelFetch(iChannel0, (ivec2(fragCoord)+iFrame*331)%1024, 0).rgb-.5);
    color /= 255.;
