// Image (image) — Refractive Polyhedron by Shane
...
// Fabrice's fork of "Integer Hash - III" by IQ: 
// https://shadertoy.com/view/4tXyWN
float hash21(vec2 f){
...
vec3 bokeh(sampler2D iCh, vec2 uv, float radius){
    ...
    for (int j = -n; j<=n; j++){
        for (int i = -n; i<=n; i++){
            ...
            //vec3 c = texture(iCh, uv + (vec2(i, j))/iRes*r, r*.7).xyz; 
            vec3 c = texture(iCh, uv + offs/iRes*r, r*iResolution.y/450.*.7).xyz; 
