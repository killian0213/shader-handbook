...
float noise( in vec2 x )
{
    ...
    vec2 f = fract(x);
	f =  f*f*(3.0-2.0*f);
	float a = textureLod( iChannel0, (i+vec2(0.5,0.5))/256.0, 0.0 ).x;
	float b = textureLod( iChannel0, (i+vec2(1.5,0.5))/256.0, 0.0 ).x;
	float c = textureLod( iChannel0, (i+vec2(0.5,1.5))/256.0, 0.0 ).x;
	float d = textureLod( iChannel0, (i+vec2(1.5,1.5))/256.0, 0.0 ).x;
...
{
	return 0.9 + 0.2*smoothstep(-0.1,0.1,sin(0.5*t));
}
...
float map( in vec3 pos, out vec4 suvw, out float info )
{
    ...
    float mushID  = hash21( floor(mushSca*pos.xz) );
...
float map2( in vec3 pos )
{
    ...
    float  rockID  = hash21( floor(rockSca*pos.xz) );
...
float raycast( in vec3 ro, in vec3 rd, out vec4 suvw, out float info )
{
...
vec3 shade( in vec3 pos, in vec3 nor, in vec3 rd, float matID, in vec3 uvw, in float info, in vec3 sunDirection, vec4 lpos[7] )
{
    vec3 rgb = vec3(0.0);
    ...
    else if( matID<2.5 )
    {
		...
		uv = fuv - 0.5 + 0.25*(-1.0+2.0*hash22( iuv ));
		uv *= 0.5 + 0.5*hash21( iuv );
		float f = 1.0 - smoothstep( 0.1, 0.3, length( uv ) );
		f *= smoothstep( 0.3, 0.31, hash21( iuv.yx*1.3 ) );
...
void moveLights( out vec4 lpos[7] )
{
    for( int i=0; i<7; i++ )
	{
		vec3 pos;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec4 suvw;
	float info;
	float t = raycast( ro, rd, suvw, info );
	...
	if( t>0.0 )
	{
        ...
        #if LIGHTRIG==0
		col = shade( pos, nor, rd, suvw.x, suvw.yzw, info, sunDirection, lpos );
		#else
  	    // super trick
		...
		col = shade( pos, nor, rd, suvw.x, suvw.yzw, info, fakeSunDirection, lpos );
        #endif
	}
    else
	{
	...
	// non illuminating fireflies	
	for( int i=0; i<32; i++ )
	{
		vec3 rrr = 1.5*sin( hash13(float(i)) + float(i)*vec3(1.2,1.1,1.7) + vec3(0.0,1.0,2.0) + 0.01*iTime);
