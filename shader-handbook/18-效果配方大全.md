...
单棵树不要画成细柱：用 **三层尖朝上的实心三角冠**（底层最宽、顶层最尖）+ 细树干。本地坐标脚在 `(0,0)`、`+y` 向上，再用 `cell` 折叠做一排。

**思路拆解**

1. 天空渐变 + 太阳盘（`exp(-r*k)`）打底。
2. `sin` 叠一层远山，和天空 `mix`。
3. 外层 `for layer`：改 `y0`（树脚）、`scl`（近大远小）、`fog`（远淡）。
4. 层内 `id = floor(x/cell)`，`fx = fract(...) - 0.5` 进本地坐标；`sdTree` = 干 ∪ 三层 `sdTriUp`。
5. 从太阳方向叠几条衰减光柱（假丁达尔）。
...
1. 只留 `layer==0`，把 `halfW` 改小，看何时变回「细柱」。
2. 加第二层并加大雾，确认空气透视。
3. 光柱里的 `sin` 改成 `hash`，避免「百叶窗」条纹。
4. 树干加宽/去掉，看剪影识别还靠不靠冠。

**易错点**

- 三角尖朝下（`base`/`tip` 搞反）→ 像吊灯。
...
```glsl
float d = min(trunk, min(c0, min(c1, c2))); // 三层尖朝上三角冠
col = mix(col, mix(tcol, sky, fog), smoothstep(0.02,-0.01,d));
```
...
海面 → 雨窗 → 远山雾 → 极光带 → 全配方合成。第 18 章词表的「深夜放映」——吃完记得回填五步卡。
...
<!-- glsl-from: examples/ch18_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch18_snack.png)
