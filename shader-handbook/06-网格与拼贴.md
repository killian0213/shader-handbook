...
整章按难度递进：方格 → 砖墙 → 六边形 → Truchet → 极坐标（年轮 / 扇形色轮 / 花瓣 / 万花筒）→ 有限重复 → 多尺度/四叉树 → 与 3D 挤出网格的关系。万花筒节有四支「真像玩具筒」的成片（`ch6_kaleido1..4`）；挤出节有四座可飞进去的小世界（`ch6_extru1..4`）。章末曼陀罗阶梯实战（6.16）把极坐标零件装成一台机器。
...
**蜂巢脉冲**：hex 格 + id 配色。会算 `hexUV` 之后，把单元格内容换成任何 2D SDF 都行。

<!-- glsl-from: examples/ch6_stage7.glsl -->
```glsl
vec2 hu = hexUV(p);
float d = hexDist(hu.xy);
```

![预览](img/ch6_stage7.png)
...
**流动 Truchet**：hash(id) 选弧，相位随时间跑——迷宫像在呼吸。

<!-- glsl-from: examples/ch6_stage8.glsl -->
```glsl
// hash(id) → tile rotation; animate phase
```

![预览](img/ch6_stage8.png)
...
### 6.7.10 万花筒（Kaleidoscope）——从「原理」到「玩具筒」

#### 先纠正一个误会

只写 `kaleido()` 再画一个圆，看起来**当然不像万花筒**——你手里只有镜子，筒里却是空的。

真玩具万花筒的观感来自三件事叠在一起：

1. **镜子**（`abs(mod(a)-半扇)`）——把一把扇形复印成整圆  
2. **扇形里塞满会动的彩色碎屑**——玻璃珠、碎纸、亮片  
3. **圆筒取景 + 整筒慢旋**——你在「往里看」，不是看一张平贴纸  

所以华丽度几乎完全取决于：**折叠之前，扇形里的内容有多密、多艳、多会动。**  
下面四个例子各自完整可跑（不是一文件切阶段），循序看完，你会把「原理」和「那种玫瑰窗幻觉」接上线。
...
vec2 kaleido(vec2 p, float n)
{
    ...
    float r = length(p);
    a = mod(a, seg);
    a = abs(a - 0.5 * seg);   // 没有 abs = 只是扇形重复；有 abs = 镜子
...
在**折叠后的坐标**里洒几十颗运动彩珠 + 星屑，外加圆筒遮罩与铜环。对称是复印机，珠子才是戏。

<!-- glsl-from: examples/ch6_kaleido1.glsl -->
```glsl
vec2 p = kaleido(uv, 8.0);
// 在 p 上画会动的彩珠 + glow —— 自动变成整筒万花
```

![万花筒① 玻璃珠](img/ch6_kaleido1.png)

**读图时注意**：单看代码循环只画了「一把扇形」里的珠；屏幕上却是八面开花。改 `n=6/10/16`，珠的密度不变，只是镜子变多。

#### ② 丝绸迷幻：折叠 × 域扭曲噪声

离散珠子换成连续的 fbm 丝绸。同一面镜子，气质从「儿童玩具」变成「教堂玫瑰窗 / 油膜」。

<!-- glsl-from: examples/ch6_kaleido2.glsl -->
```glsl
vec2 p = kaleido(uv, 10.0);
vec2 q = p + 0.55 * fbm(...);   // 在折叠空间里拧
col = palette(fbm(q));
```

![万花筒② 丝绸](img/ch6_kaleido2.png)

**对照 ①**：① 靠粒子密度，② 靠场的褶皱。两者都说明——**万花筒不生产美，只放大你塞进去的美。**

#### ③ 霓虹电路：Truchet 塞进镜子

把本章 6.6 的圆弧 Truchet 铺进折叠空间，再叠辉光。气质靠近语料 `000273-7lKSWW-Truchet_Kaleidoscope_FTW`（mrange）——你会第一次强烈感到「网格章 × 万花筒 = 科幻玫瑰窗」。

<!-- glsl-from: examples/ch6_kaleido3.glsl -->
```glsl
vec2 p = kaleido(uv, 12.0);
// 在 p*scale 上做 Truchet 圆弧 + neon glow
```

![万花筒③ 电路](img/ch6_kaleido3.png)

**动手**：把 `scale` 从 6 改到 14，线变密；把第二层反向滚动关掉，看层次变薄。

#### ④ 双层反旋玫瑰窗（压轴）

外层 8 镜慢旋、内层 12 镜反旋，瓣/弧/点饰叠加——镜数与转速不同会产生「拍频」，图案像活的。再加星尘与暗角，这才是「华丽」该有的密度。

<!-- glsl-from: examples/ch6_kaleido4.glsl -->
```glsl
col  = layer(uv, 8.0,  +iTime*0.15, ...);
col += layer(uv, 12.0, -iTime*0.22, ...); // 反旋、不同镜数
```

![万花筒④ 双层](img/ch6_kaleido4.png)
...
| ① | 运动彩珠 | 内容密度决定观感 |
| ② | 域扭曲噪声 | 连续场也能万花 |
| ③ | Truchet 电路 | 本章网格零件直接进筒 |
| ④ | 双层几何饰片 | 不同 `n`/转速制造「活」 |

语料狂欢版继续读：`000273`（Truchet 万花筒）、以及任何把噪声/SDF 放进 `kaleido` 的作品——读法永远是：**先找扇形里的内容函数，再看镜子参数 `n`。**
...
**较难 · 四叉树城市灯**：用循环模拟递归细分，hash 决定是否继续劈格。霓虹边 = 单元格边界 SDF。

<!-- glsl-from: examples/ch6_stage9.glsl -->
```glsl
// for level: if hash(id)>split → subdivide
```

![预览](img/ch6_stage9.png)
...
## 6.10 与 raymarch 挤出网格的关系——四个可飞进去的世界

到目前为止都是 2D。语料里大量「科幻地板 / 迷宫 / 晶格世界」其实就三步：

```
2D 网格 SDF 或高度  →  挤出成 3D  →  raymarch
```

这里会**稍微超纲**（正式骨架在第 7 章），但只借用最小 `march / 法线 / Lambert`，好让你立刻尝到「格子站起来」的乐趣。四个例子各自完整，按难度递进——请分开打开，不要合成阶段开关。

### 6.10.1 挤出公式：把「平的」变成「柱」

若 `d2 = sdPattern(p.xz)` 是平面上的有符号距离，半高为 `h`：
...
- `w.x`：水平方向还在不在 2D 图案里；
- `w.y`：竖直方向有没有超出高度 `[-h,h]`（常改成从地面往上长：`abs(p.y - h) - h`）；
- 外面用 `length(max(w,0))`，里面用 `min(max(...),0)`——和 `sdBox` 同一家族。

**贵的是 2D 图案，不是挤出本身。** 图案丑，挤出来也只是丑柱林；图案好看，挤完再加光就成世界。

### 6.10.2 ① 圆点阵 → 柱林（最小完整闭环）

先做最傻的 2D：`fract` 格子里一个圆。挤出成柱，相机绕飞，Lambert 上色。读完你应能指着代码说：「这是格子」「这是挤出」「这是 march」。

<!-- glsl-from: examples/ch6_extru1.glsl -->
```glsl
float d2 = sdCircleGrid(p.xz, ...);
float pillars = opExtrusion(d2, p.y - 0.6, 0.6);
return min(p.y, pillars);
```

![圆点柱林](img/ch6_extru1.png)

**动手**：把圆换成 `sdBox2`，柱林变方塔；给 `hash(id)` 乘高度，立刻参差不齐。

### 6.10.3 ② 六边形高度场：科幻地砖

每格 `hash(id)` → 不同挤出高度 + 软阴影 + 边缘一点霓虹。这就是「科幻地板」最小成片：结构来自 hex，差异来自哈希，立体来自挤出。

<!-- glsl-from: examples/ch6_extru2.glsl -->
```glsl
float h = 0.08 + 0.55 * hash21(hexId);
// dHex = 六边形 2D 距离；再 opExtrusion 到高度 h
```

![六边地砖](img/ch6_extru2.png)

对照 6.5 的 2D 蜂巢：那里只是填色；这里同一套 id，变成了可以投下阴影的城市肌理。

### 6.10.4 ③ Truchet 浮雕迷宫

把 6.6 的圆弧 Truchet 当作 2D 距离，挤成矮墙，相机掠射扫过——管道感来自图案，立体感来自挤出。符号取反可以在「凹槽管道」和「凸起浮雕」之间切换。

<!-- glsl-from: examples/ch6_extru3.glsl -->
```glsl
float d2 = -truchet2D(p.xz); // 符号决定凹/凸
float relief = opExtrusion(d2, p.y - 0.18, 0.18);
```

![Truchet 迷宫](img/ch6_extru3.png)

**易错**：`d2` 线宽（本例 `- 0.12`）太窄 → 墙像纸片；太宽 → 糊成实体板。先在 2D 把 Truchet 调到愿意截图，再挤。

### 6.10.5 ④ 扭曲晶格走廊（压轴）

方格柱林 + 沿飞行方向的 sine 域扭曲 + 稀疏 `hash` 占格 + 霓虹描边 + 雾。你在「飞进」一个格子世界——这正是 Shane 类作品的趣味核，只是参数更短。

<!-- glsl-from: examples/ch6_extru4.glsl -->
```glsl
q.x += 0.35 * sin(q.z * 0.45 + iTime); // 先扭曲再进格子
// 稀疏占格 → 挤出柱 → 飞穿相机
```

![晶格走廊](img/ch6_extru4.png)

**设计顺序（务必遵守）**：

1. 先在 2D（或静止 3D 顶视）把格子图案调到愿意截图；  
2. 再挤出；  
3. 最后加 warping / 相机路径 / 霓虹与雾。  

反过来做会在一堆 `mat2` 和闪烁里迷路。

### 6.10.6 Shane 语料：下一步读什么

本章四个例子吃饱之后，按这个顺序进语料（仍然用第 0 章读法：先找 `map`，分开「格点 / 挤出 / warp」）：

| 作品 | 看什么 |
|---|---|
| `000147-ttVBzd-Extruded_Truchet_Pattern` | Truchet 浮雕原教旨 |
| `000329-4td3zj-Raymarched_Hexagonal_Truchet` | 六边 Truchet 3D |
| `000373-wtfBDf-Warped_Extruded_Skewed_Grid` | 扭曲 + 斜切 + 挤出 |
| `000205-4l2cD3-Dual_3D_Truchet_Tiles` | 双层 3D Truchet |
| `000324-MsVfz1-Neon_Lit_Hexagons` | 霓虹六边体 |
| `000600-Mld3Rn-Perspex_Web_Lattice` | Voronoi 晶格网 |

2D 基础不扎实就冲这些会被绕晕——所以它们放在噪声之后、raymarch 专章之前；你现在已经有四座「自制小桥」可以走过去。
...
7. 依次跑 `ch6_kaleido1..4`：只改内容、不改镜子，体会「华丽从哪来」。
8. 把 `ch6_kaleido1` 的珠子换成短线段，做成「碎镜片」筒。
9. 用 `clamp(round(p.x), -2.0, 2.0)` 做 5 根柱子的 2D 剖面。
10. 打开 `000141`，删到只剩单层 Truchet，再一层层加回细分。
11. 依次跑 `ch6_extru1..4`：从柱林走到晶格走廊；把 `extru1` 的圆换成方盒。
12. 给方格线加上 `fwidth` 抗锯齿，缩远看是否还闪。
13. 读 `000373` / `000147`，对照你的 `extru3`/`extru4` 找「格点 / 挤出 / warp」三行。
14. 把 6.16 曼陀罗的 `sdPetal` 换成星形或十字，整张图换一种气质。
...
同心环 → 极坐标折叠 → 花瓣 SDF → 调色板 → 辉光呼吸。网格章的拼贴语言一次开成会转的曼陀罗。
...
<!-- glsl-from: examples/ch6_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch6_snack.png)
...
- **极坐标重复 / 万花筒** 用 `atan` + `mod`（+ `abs` 镜像）做旋转对称；**华丽来自扇形里的内容**（见 `ch6_kaleido1..4`），不是来自 `fold` 公式本身。
- **有限重复** 用 `clamp(round)`，避免 `fract` 在全空间无穷复制。
- **四叉树 / 多尺度** 是在哈希控制下切换格子粒度（`000141`、`000209`）。
- 3D 酷炫网格多是 **2D 图案挤出 + raymarch**；路径见 `ch6_extru1..4`（柱林 → hex 地砖 → Truchet 迷宫 → 晶格走廊），再读 Shane 系列。
