...
3D SDF 图元公式默认描述「以原点为中心」的形状。想摆到世界上某处，永远是 `f(p - center)`，不是改半径公式。本例用地面 + 两球一盒验证：同一套 raymarch，只改 `map`。

**思路拆解**

1. `map` 里每个物体各自 `p - center`，再 `min`（或带材质的 opU）。
2. 地面用 `p.y`（过原点的水平面）。
3. 命中后用有限差分求法线，Lambert + 简易软阴影上色。
4. 读代码时先标出「哪一行是域变换、哪一行是距离」。
...
1. 把球心从 `(-1.6,0.75,0)` 改到 `(0,2,0)`，确认「移的是坐标」。
2. 删掉盒子那一支，看 `map` 变简单后阴影是否还对。
3. 把软阴影步数砍半，观察接触阴影变硬。

**易错点**

- 写成 `sdSphere(p, r) + center` 之类——那不是平移。
- 法线 eps 随 `t` 放大不够 → 远景法线抖。
...
并集是 `min`，差集是 `max(dA, -dB)`。本例：方盒 ∪ 圆环，再挖掉一个球咬痕。差集参数顺序写反会「长肉」而不是挖洞——这是造型里最高频的笔误。

**思路拆解**

1. 先 `d = min(box, torus)` 得到并集外壳。
2. 再 `d = max(d, -bite)`：咬痕球内部变成「禁止区域」。
3. 用材质 ID 时不要裸 `min`，改用比较 `a.x < b.x` 的 `opU`。
...
1. 暂时注释掉差集，只看并集轮廓。
2. 把 `-bite` 改成 `+bite` 或交换参数，故意看一次「长肉」。
3. 咬痕球挪到接缝处，看布尔对法线的影响（硬边）。

**易错点**

- `opSubtraction(a,b)` 社区两种参数顺序并存，读语料先确认。
- 差集后接触阴影容易漏光，偏置要略加大。
...
有机接缝靠平滑并集 `smin`。胶囊腿 + 球身 + 头：把任意一处改回硬 `min`，接缝立刻变尖——这是理解 k 的最快实验。

**思路拆解**

1. 各部件仍用域平移摆好。
2. `d = smin(body, leg, k)`，再 `smin` 头；k 越大融合越胖。
3. 注意：强 smin 会缩短有效距离，march 可能需更保守或更多步。
...
1. 把一处 `smin` 改 `min`，截图对比接缝。
2. 把 k 从 0.05 拉到 0.35，看「融化」。
3. 试 `smax` 做平滑差集（挖软洞）。

**易错点**

- k 过大 → Lipschitz 变差 → 表面痤疮或漏穿。
- smin 后材质 ID 混叠：需要按权重混合 ID 或接受硬切 ID。
...
无限柱廊不是画 100 根柱，而是 `p.xz = mod(p.xz+0.5*s,s)-0.5*s` 把空间折进一个格子，只对**一根**圆柱求距离。

**思路拆解**

1. 选重复周期 `s`（格子边长）。
2. 折叠 xz（通常不折 y，否则柱被切成段）。
3. 在基本域里写 `sdCylinder` / `sdBox`。
4. 地面可单独算，不要被 mod 弄碎。
...
1. 只折叠 x，做成「一面墙的柱列」。
2. 给每格加 `hash(id)` 微扰高度（先 `floor` 取 id 再 mod）。
3. 周期太小会自交，march 不稳——加大 s。

**易错点**

- 忘了 `-0.5*s` 居中 → 柱贴在格子边上。
- 对已 mod 的坐标又做一次大平移，基本域对不齐。
...
域扭曲：先按高度旋转 xz，再喂 `sdBox`。改的是输入点，不是输出距离。twist 非等距 → 结果只是近似 SDF，心里留 fudge。

**思路拆解**

1. `ang = p.y * k`；`p.xz *= rot(ang)`。
2. 再 `sdBox(p, b)`（或圆柱）。
3. 命中阈值略放宽，或距离乘保守系数。
...
1. k=0 还原直柱，确认骨架。
2. 只扭上半段：`ang *= smoothstep(...)`。
3. 对比 bend（绕别的轴弯）。

**易错点**

- 扭太猛 → 黑洞/漏面；减小 k 或加大步数。
- 法线仍用差分可以，但高扭曲区会噪。
...
建筑造型 = 有序硬布尔，不是一锅 smin。推荐搭法：台阶 → 墙体 → 四柱 → 山墙 → 半球穹顶。每一步都能单独渲染验收。

**思路拆解**

1. 台阶：`for` 叠若干扁盒，或一根楼梯 SDF。
2. 墙/山墙：`sdBox` 差集出门洞。
3. 四柱：域对称或四次平移 `min`。
4. 穹顶：`max(sphere, -平面)` 取上半球。
5. 最后才加地面与简单光照。
...
1. 先只留台阶+地面，再逐段取消注释。
2. 门洞差集顺序反了会堵死——对照 stage2。
3. 给山墙加一点倾角（域剪切）更有神庙感。
...
## 课间餐点 · 月下微型神庙：CSG 五层封顶

> 阶梯练零件；这里把本章词汇一次端上桌。约 2 分钟看完一轮自动轮播——像课间买的糖，甜，但糖纸上写满了今天的知识点。

台基 → 柱廊 → 山墙 → 穹顶 → 月光雾影。看着一座建筑从盒子长出来——造型章的秀场。
...
<!-- glsl-from: examples/ch8_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch8_snack.png)
