...
## 12.18 阶梯实战：从硬阴影球到镜面·玻璃小场景

路径追踪要 Buffer 累积；网页版单 Pass 先把**解析求交 + 反射/折射**练成片。五段代码在 `examples/ch12_stage1..5.glsl`。

真·渐进式 PT（噪声→收敛）请到 Shadertoy 开 Buffer 存累计颜色；本章阶梯保证你**随时能看见反射/玻璃在干什么**。
...
**接着**：把弹跳次数接到 `iMouse`；给地面加粗糙反射；去读 Demofox 的 PT 教程对照累积。
