...
单 Pass 没有「上一帧纹理」。用 iTime 相位 + hash 模拟最近 N 个笔迹点发光，骗出拖尾感。真拖尾 = Buffer 自反馈。

**思路拆解**

1. 当前笔尖：鼠标或螺旋公式。
2. for i in 0..N：年龄 age=i/N，位置沿轨迹回退。
3. `glow *= 1-age` 累加。
...
同一轨迹取多个相位（延迟），每个画一个半透明笔尖——像示波器余晖。仍是解析假历史，但更「电子」。
...
#### 可跑精读 · 阈值提取亮部（Bloom 前半）

Bloom 管道：先抽出亮部 → 模糊（真多 Pass）→ 加回。单 Pass 用宽核近似模糊，先把「阈值」手感做对。

**思路拆解**

1. `bright = max(scene - thr, 0)`。
2. 对 bright 做廉价模糊或散射。
3. `col = scene + bloom*gain`。
...
用时间与 hash 模拟玩家、金币、HUD——真游戏要把状态存 Buffer/CPU。这里练的是「可读 UI + 碰撞反馈」的视觉核。

**思路拆解**

1. 玩家圆位置：键鼠或自动巡游。
2. 金币：格子 hash + 收集后隐藏（时间门）。
3. HUD：条/分用 `smoothstep` 画。
...
真反馈：Buffer 读上一帧再经 UV 变换写出。网页单 Pass 用「历史相位图」近似 zoom 拖影味道。

**思路拆解**

1. 想象 uv 每次向中心缩放一点再混原图。
2. 本例用多层缩放采样伪积累。
3. 真 Shadertoy：BufferA 读 BufferA。
...
反馈时 R/G/B 用不同 UV 偏移，得到棱镜拖影。常用于故障艺术与音乐可视化。
...
`frame = floor(iTime * fps)` 当离散时间，所有动画吃 frame 而非连续 t——停格、Slideshow、游戏逻辑步进的基础。

**思路拆解**

1. 选 rate（如 2 fps）。
2. `ft = frame / rate` 或直接用 frame 当种子。
3. 粒子/颜色随 ft 变。
...
1. fps=12 对 60，看差异。
2. 用 frame 驱动 hash 换色。
3. 游戏逻辑里用它做节拍（stage8）。

**易错点**

- 混用 iTime 与 frame 导致一部分丝滑一部分跳。
...
基色 + 光晕：注意线性空间相加，再 tonemap。本例强调合成顺序与增益，而不是完美高斯。
...
单点 → 多段残影 → 阈值光晕 → RGB 分离 → 完整光绘。多 Pass 章在网页上的「味道合菜」（真反馈需 Buffer）。
...
<!-- glsl-from: examples/ch13_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch13_snack.png)
