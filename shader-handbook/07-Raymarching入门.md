...
圆环比单球更有「场景」。文件顶 `HEAT_OVERLAY`：把步数映射成颜色——调试探针本身可以是作品。
...
1. 开热力找贵的像素（边缘/缝）。
2. 加大 MAX_STEPS 看热力变冷。
3. 把环换成神庙（第 8 章）。

**易错点**

- 热力忘 tonemap 分离 → 调色被污染。
...
Raymarching 最小核：相机射线逐步累加 `map` 距离，看是否在预算内碰到表面。先出黑白剪影，再谈法线与光。

**思路拆解**

1. `ro/rd` → `t += map(ro+rd*t)`。
2. `map < eps` 命中；`t > MAX_DIST` 天空。
3. 命中涂白/物体色，否则背景色。
...
1. 把球换成盒，确认只改 map。
2. MAX_STEPS 砍到 16，看破洞。
3. 输出 `t` 灰度当深度探针。

**易错点**

- 步进取 `map*0.5` 过保守会变慢；取 `>1` 易漏。
- eps 固定过小远景抖。
...
有限差分估法线：`n = normalize(map(p+e)-map(p-e))`。`n*0.5+0.5` 是最便宜的形体调试色。

**思路拆解**

1. 命中后算 n。
2. `fragColor = vec4(n*0.5+0.5,1)`。
3. 检查接缝/破面是否突然变色。
...
1. 故意把 map 写错符号，看法线内外翻。
2. 下一步才加 Lambert。

**易错点**

- 对非 SDF（扭曲过度）差分会噪——先减小扭曲。
...
`map` 返回 `vec2(d, matId)`：球与地面分开着色；天空用 rd.y 渐变。Lambert = `albedo * max(N·L,0)`。
...
从着色点沿 L 再 march：记录过程中 `k*h/t` 的最小值，得到半影。比硬阴影多一点点成本，接触处立刻可信。

**思路拆解**

1. 原点 `p+n*bias`。
2. 行进中追踪 `res = min(res, k*h/t)`。
3. 漫反射乘 res。
...
沿法线短距离采样 map，缝隙更暗——球才「坐」在地上。常和软阴影一起用。
...
`col = mix(fogColor, col, exp(-d*dens))`：远球融进雾。几乎不改几何，场景立刻有「空气」。

**思路拆解**

1. 用命中 t（或平面距离）当 depth。
2. 选雾色接近地平线天空。
3. 可加高度雾：`dens *= exp(-h*...)`。
...
同一骨架上堆：多物体、棋盘地、tonemap、暗角。证明「好看」多半来自光照与后期，不是更复杂 SDF。

**思路拆解**

1. map 里多颗球。
2. 地面用棋盘 albedo。
3. ACES/filmic + vignette。
...
1. 去掉 tonemap 看 HDR 裁切。
2. 只留一球确认光仍对。
3. 开 HEAT 对照 stage8。

**易错点**

- 先 tonemap 再加光 → 发灰；保持线性算光。
...
## 课间餐点 · 雾中小剧场：Raymarch 五幕连演

> 阶梯练零件；这里把本章词汇一次端上桌。约 2 分钟看完一轮自动轮播——像课间买的糖，甜，但糖纸上写满了今天的知识点。

剪影 → 法线 → Lambert → 软阴影 → 晨雾打磨。同一球+地面，五幕实时解锁——第 7 章口诀的电影预告片。
...
<!-- glsl-from: examples/ch7_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch7_snack.png)
