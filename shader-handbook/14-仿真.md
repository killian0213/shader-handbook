...
## 14.11 阶梯实战：粒子、涟漪与 RD 的视觉直觉

网页阅读器没有 float Buffer，完整 Gray-Scott / 流体请到 Shadertoy 按 14.3–14.5 节搭建。

下面四段让你**先看见仿真章该有的画面**，并标明真多 Pass 接法。文件在 `examples/ch14_stage1..4.glsl`。

### 阶段 1：解析粒子场

位置由 id+iTime 算出；真交互粒子把状态写入 Buffer。
...
![阶段2](img/ch14_stage2.png)


### 阶段 3：RD 形态切片

域扭曲噪声模拟 spots/stripes。
...
![阶段4](img/ch14_stage4.png)


### 回头看

| 阶段 | 网页上 | Shadertoy 真接法 |
|---|---|---|
| 1 | 解析粒子 | Buffer 存位置/速度 |
| 2 | 解析涟漪 | 双历史波方程 |
| 3 | RD 形态直觉 | Gray-Scott float+nearest |
| 4 | 成片 | 任选上面一个接后期 |

**接着**：打开语料里的 Gray-Scott，把阶段 3 的上色思路贴到 Image Pass。
