...
真 Gray-Scott 要多 Pass 存两浓度。单 Pass 用域扭曲 fbm 做出 spots/stripes 相，先立住「纹样语言」。
...
**Life 味道**（单 Pass）：`floor(iTime)` 当代数 + hash 邻域。真 Game of Life = Buffer + nearest + texelFetch。

#### 可跑精读 · CA 味道 · 世代哈希

真 CA 读邻格状态；单 Pass 用 `generation = floor(iTime*rate)` + 邻域哈希规则，做出闪烁生命味道。

**思路拆解**

1. 格子 id；generation 作种子。
2. 伪邻域和 → birth/survival 规则。
3. 单元格着色。
...
1. rate 极慢，盯一代。
2. 改 birth 阈值。
3. 对照 stage10 更平滑的质量版。

**易错点**

- 无持久状态 → 不能真正稳定静物；那是 Buffer 的活。

<!-- glsl-from: examples/ch14_stage9.glsl -->
```glsl
// generation = floor(iTime * rate); neighbor-ish hash rules
```

![预览](img/ch14_stage9.png)

**SmoothLife 味道**：连续场 + 出生/存活带 → blob 融合分裂，比方格 Life 更有机。

#### 可跑精读 · 平滑生命/质量场

邻域「质量」用平滑距离核，birth/survival 用 smoothstep——看起来像软细胞，而不是方格闪烁。

**思路拆解**

1. 对邻格累加质量。
2. 两道 smoothstep 规则出 alpha。
3. 色随质量变。
...
<!-- glsl-from: examples/ch14_stage10.glsl -->
```glsl
// birth/survival smoothsteps on neighborhood mass
```

![预览](img/ch14_stage10.png)
...
**较难延展 · 帘幕 + curl 染料**：鼠标推挤丝带；背景用 curl-noise 假装半拉格朗日平流。真流体见上文 Buffer 拓扑。

#### 可跑精读 · 半拉格朗日墨带

鼠标拖一条速度带，颜色沿 `-v` 回溯采样——半拉格朗日 advection 的视觉核。真流体还要投影去散度。
...
<!-- glsl-from: examples/ch14_stage12.glsl -->
```glsl
// ribbon vs iMouse; dye = fbm(p - curl*t)
```

![预览](img/ch14_stage12.png)
...
N 个粒子位置 = `f(id, iTime)`，无需 Buffer。先做出「成千上万个点在动」的密度感，再谈交互状态。

**思路拆解**

1. `hash(id)` 出种子：速度、相位、色。
2. 轨道/噪声位移算 cen。
3. `exp(-dist*k)` 累加发光。
...
1. N=10 看清单点。
2. 改成上升雪花：`y = fract(t*spd+seed)`。
3. 对照 stage7 curl 流场粒子。
...
速度取噪声梯度的旋度（转 90°），粒子沿场走——自然无源（看起来「不可压缩」）。
...
**假旗帜**：网格点 sin 位移 + 简易光照。建立「质点网格在动」的直觉；真 Verlet = 位置双缓冲 + 距离约束迭代。

#### 可跑精读 · 布料网格味道

规则格点 + 风扰动位移，用面法线打光——布料/旗的最小可读模型（非真弹簧积分）。
...
<!-- glsl-from: examples/ch14_stage11.glsl -->
```glsl
// grid[i,j] displaced by wind sin; light with face normal
```

![预览](img/ch14_stage11.png)
...
`sin(k*r - ωt)*exp(-damp*r)`：一颗石子的圆波。多源叠加即干涉（stage5）。
...
两颗以上石子：`w = Σ sin(k*|p-src_i|-ωt)`。鼠标可当一颗活源。
...
同一反应扩散味道核，改阈值与各向异性扭曲，从斑点变成条虫。调参比换算法重要。

**思路拆解**

1. 各向异性：`q.x` 与 `q.y` 不同缩放。
2. 阈值带变窄 → 条纹。
3. 时间在扭曲域里慢慢流。
...
`field = Σ r²/(dot(p-c,p-c)+ε)`，阈值 iso 成团——软体融合的经典。

**思路拆解**

1. 多中心运动。
2. 求 field；`smoothstep(iso-w, iso+w, field)`。
3. 用场梯度当假法线打光。
...
## 课间餐点 · 活水墨池：粒子·涟漪·curl 合菜

> 阶梯练零件；这里把本章词汇一次端上桌。约 2 分钟看完一轮自动轮播——像课间买的糖，甜，但糖纸上写满了今天的知识点。

粒子场 → 圆波干涉 → metaball → curl 墨带 → 全部叠进一池。仿真章语法的趣味拼盘。
...
<!-- glsl-from: examples/ch14_snack.glsl -->
```glsl
// snackStage() 解锁图层 · 底部黄条 = 当前阶段 · 鼠标拖拽 scrub
```

![课间餐点](img/ch14_snack.png)
